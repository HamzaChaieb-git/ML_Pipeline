...
# Attempt to import MlflowSpanExporter, fall back to MLflow tracing if unavailable
try:
...
def run_enhanced_pipeline(train_file: str, test_file: str) -> None:
    ...
    try:
        ...
        with mlflow.start_run(run_name=f"Pipeline_v{model_version}") as run:
            ...
            with tracer.start_as_current_span("system_info_logging") if OPENTELEMETRY_AVAILABLE else tracing.start_span("system_info_logging") as system_span:
                log_system_info()
            ...
            with tracer.start_as_current_span("data_preparation") if OPENTELEMETRY_AVAILABLE else tracing.start_span("data_preparation") as data_span:
                print("📊 Preparing data...")
                X_train, X_test, y_train, y_test = process_data(train_file, test_file)
                mlflow.log_metric("train_samples", len(X_train))
                mlflow.log_metric("test_samples", len(X_test))
                if data_span and OPENTELEMETRY_AVAILABLE:
                    data_span.set_attribute("train_samples", len(X_train))
                    data_span.set_attribute("test_samples", len(X_test))
                print("✅ Data preparation complete")
            
            # Model training phase
            with tracer.start_as_current_span("model_training") if OPENTELEMETRY_AVAILABLE else tracing.start_span("model_training") as training_span:
                print("🔧 Training model...")
                model = train_xgb_model(X_train, y_train, model_version=model_version)
                print("✅ Model training complete")
            
            # Model evaluation phase
            with tracer.start_as_current_span("model_evaluation") if OPENTELEMETRY_AVAILABLE else tracing.start_span("model_evaluation") as eval_span:
                print("📈 Evaluating model...")
                metrics = evaluate_xgb_model(model, X_test, y_test)
                if eval_span and OPENTELEMETRY_AVAILABLE:
            ...
            # Log model
            with tracer.start_as_current_span("model_logging") if OPENTELEMETRY_AVAILABLE else tracing.start_span("model_logging") as logging_span:
