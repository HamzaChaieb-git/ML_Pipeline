...
st.markdown('<div class="stText">Predict customer churn with a simple, user-friendly interface.</div>', unsafe_allow_html=True)
    ...
    if submit_button:
        ...
        try:
            ...
            fig = go.Figure(go.Indicator(
                ...
                title={'text': "Churn Probability (%)", 'font': {'size': 14, 'color': '#2c3e50'}},
                gauge={'axis': {'range': [0, 100], 'tickcolor': '#7f8c8d', 'tickwidth': 1, 'tickfont': {'size': 12, 'color': '#7f8c8d'}},
                       'bar': {'color': "#3498db"},
                       'steps': [
                           {'range': [0, 50], 'color': "#f5f7fa"},
                           {'range': [50, 100], 'color': "#e9ecef"}],
                       'threshold': {'line': {'color': "#e74c3c", 'width': 3}, 'thickness': 0.75, 'value': 80}},
                number={'valueformat': ".1f", 'font': {'size': 24, 'color': '#2c3e50'}}
            ))
            fig.update_layout(
            ...
            st.markdown(f'<div class="stError">Error making prediction: {str(e)}. Please check if the FastAPI server is running at fastapi:8000.</div>', unsafe_allow_html=True)
