...
def create_evaluation_artifacts(
    ...
    artifact_dir = os.path.join(
        "artifacts", "evaluation", datetime.now().strftime("%Y%m%d_%H%M")
    )
    ...
    fig.add_trace(go.Scatter(x=fpr, y=tpr, name="ROC curve"), row=1, col=1)
    fig.add_trace(
...
def create_summary_report(metrics: Dict[str, float], artifact_dir: str) -> None:
    ...
    report = {
        ...
        "model_recommendation": (
            "production"
            if metrics["accuracy"] > 0.85 and metrics["roc_auc"] > 0.85
            else "staging"
        ),
    }
...
def evaluate_model(model: Any, X_test: Any, y_test: Any) -> Dict[str, float]:
    ...
    metrics.update(
        {
            "balanced_accuracy": (metrics["recall_class_0"] + metrics["recall_class_1"])
            / 2,
            "precision_recall_ratio": (
                metrics["precision"] / metrics["recall"] if metrics["recall"] > 0 else 0
            ),
            "false_positive_rate": metrics["false_positives"]
            / (metrics["false_positives"] + metrics["true_negatives"]),
            "false_negative_rate": metrics["false_negatives"]
            / (metrics["false_negatives"] + metrics["true_positives"]),
            "positive_predictive_value": metrics["precision"],
            "negative_predictive_value": metrics["true_negatives"]
