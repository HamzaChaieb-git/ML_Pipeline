...
def test_prepare_data_categorical_encoding(sample_data):
    ...
    for feature in categorical_features:
        assert X_train[feature].dtype == int, f"{feature} in X_train should be encoded as integers"
        assert X_test[feature].dtype == int, f"{feature} in X_test should be encoded as integers"
        assert X_train[feature].min() >= 0, f"{feature} in X_train should have non-negative values"
        assert X_test[feature].min() >= 0, f"{feature} in X_test should have non-negative values"
