...
class TrainingArtifacts:
    ...
    def log_data_stats(self, X: pd.DataFrame, y: np.ndarray) -> None:
        ...
        stats = {
            "dataset_info": {
                ...
                "missing_values": {
                    str(k): int(v) for k, v in X.isnull().sum().to_dict().items()
                },
            },
            "feature_stats": {
                col: {
    ...
    def create_correlation_matrix(self, X: pd.DataFrame) -> None:
        ...
        if len(numeric_cols) > 0:
            ...
            mlflow.log_artifact(
                os.path.join(self.artifact_dir, "correlation_matrix.png")
            )
            mlflow.log_artifact(
                os.path.join(self.artifact_dir, "correlation_matrix.html")
            )
...
def train_model(
    X_train: Union[pd.DataFrame, Any], y_train: Any, model_version: str = "1.0.0"
) -> xgb.XGBClassifier:
    """
    ...
    with open(
        os.path.join(artifacts.artifact_dir, "parameter_descriptions.json"), "w"
    ) as f:
        json.dump(param_desc, f, indent=4)
    mlflow.log_artifact(
        os.path.join(artifacts.artifact_dir, "parameter_descriptions.json")
    )
    ...
    # Train model
    model.fit(X_train_final, y_train_final, eval_set=eval_set, verbose=True)
    ...
    if isinstance(X_train, pd.DataFrame):
        ...
        mlflow.log_artifact(
            os.path.join(artifacts.artifact_dir, "feature_importance.png")
        )
        mlflow.log_artifact(
            os.path.join(artifacts.artifact_dir, "feature_importance.html")
        )
    ...
    mlflow.xgboost.log_model(
        ...
        signature=signature,
        input_example=(
            X_train.iloc[:5] if isinstance(X_train, pd.DataFrame) else X_train[:5]
        ),
